<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <jsp:include page="/WEB-INF/pagine/default/head.jsp">
        <jsp:param name="title" value="Contatti"/>
        <jsp:param name="style" value="styleHeader,styleFooter,styleFaq"/>
        <jsp:param name="script" value="header,footer"/>
    </jsp:include>
    <link rel="stylesheet" href="./css/styleFaq.css">
</head>

<body>
<jsp:include page="default/header.jsp"/>

<div class="FAQ">

    <h1 class="titoloFAQ">FaQ - Domande Frequenti</h1>

    <h1>Come faccio a conoscere la data di scadenza di un prodotto?</h1>

    Tutti i prodotti presenti sul sito FAIMED.IT hanno una scadenza dai i 12 mesi in poi.

    I prodotti a scadenza breve vengono segnalati o inseriti in una categoria specifica.

    Per ricevere più informazioni inviare una E-mail a assistenza@faimed.it






    <h1>Come posso essere sicuro di aver effettuato l'acquisto in modo corretto?</h1>

    Una volta effettuato l'ordine riceverai un messaggio di conferma con un numero ordine al tuo indirizzo E-mail. Se non ti arriva, contatta il servizio clienti alla seguente E-mail: assistenza@faimed.it .



    <h1>Non ho ricevuto la E- mail di conferma dell’ordine.</h1>

    Nessun problema. Forse hai digitato male il tuo indirizzo mail o la nostra mail di conferma è finita nello SPAM. Invia una mail ad assistenza@faimed.it indicando il tuo nome e cognome e i prodotti acquistati



    <h1>Ho dimenticato di inserire un prodotto come posso fare?</h1>

    Se hai finalizzato un ordine ed effettuato anche il pagamento. Hai tempo fino a 12 ore per inserire un prodotto all’ordine cliccando qui



    <h1>Costo della spedizione?</h1>

    Le spese di consegna sono gratuite in tutta Italia per ordini superiori a 69,99€ .

    Se si effettua un acquisto di un importo minore a 69,99€ la spedizione ha un costo di 5,00€.

    Il corriere che utilizziamo per le spedizioni Nazionali è GLS. Se sei a conoscenza di problemi che ha GLS sul tuo territorio e preferisci usufruire di un altro corriere contattaci alla mail assistenza@faimed.it per comunicarci le tue preferenze.

    *In caso di prodotti liquidi, particolarmente pesanti e/o voluminosi, Faimed si riserva il diritto di contattare il cliente, per comunicare eventuali adeguamenti dei costi di spedizione.



    <h1>Non riesco ad effettuare il pagamento con carta di credito .</h1>

    Verifica la connessione internet e che tutti i campi siano stati inseriti in modo corretto.

    Può essere stato raggiunto il limite di spesa della carta o che sia scaduta, in questo caso contattare la propria banca.



    <h1>Ho bisogno di un grande quantitativo di un prodotto, come verifico le quantità disponibili?</h1>

    Tutti i prodotti presenti su FAIMED.IT hanno una propria scheda.

    Per conoscere la quantità di un prodotto cliccare su “DETTAGLI PRODOTTO”. Se hai necessità di ulteriore quantitativo del prodotto indicato, puoi contattarci e provvederemo a integrare l’ordine nel più breve tempo possibile.



    <h1>Ho indicato un indirizzo errato. Come posso modificarlo?</h1>

    Per richiedere la modifica di un indirizzo inviare una E-mail a assistenza@faimed.it indicando il numero ordine e l’indirizzo corretto.

    Questa procedura è la stessa da utilizzare anche per ordini già spediti. Saremo noi a comunicare al corriere il nuovo indirizzo.



    <h1>Lo scontrino relativo all’acquisto dove posso trovarlo?</h1>

    Tutti gli scontrini vengono inseriti all’interno del pacco.

    Si prega di conservare lo scontrino per eventuali resi o sostituzioni.

    N.B. In caso di reso, possiamo accettare merce solo accompagnata dallo scontrino fiscale da noi emesso. È obbligatorio quindi conservare lo scontrino, nel caso si volesse procedere al reso parziale o totale di prodotti.



    <h1>Il prodotto che mi interessa risulta terminato. Tornerà disponibile?</h1>

    I prodotti presenti su FAIMED.IT vengono periodicamente riordinati. Se non trovi un prodotto invia una mail al servizio clienti: assistenza@faimed.it .



    <h1>Posso fare un ordine telefonico?</h1>

    Certo, puoi acquistare i tuoi prodotti anche al telefono. Contattaci al (+39) 3920591662.

    Siamo quotidianamente al tuo servizio anche telefonicamente. Se il numero di telefono risulta spento/non raggiungibile o squilla senza risposta, è perché siamo impegnati in un'altra conversazione. Se non vuoi richiamare, lasciaci un messaggio su WhatsApp con la tua richiesta e ti richiameremo il prima possibile.



    <h1>Ho ricevuto l’e-mail di spedizione ordine ma non trovo il numero per il Tracking. Come posso riceverlo?</h1>

    Se il tuo ordine è stato spedito ma non hai il numero spedizione, contatta il servizio clienti indicando il numero ordine. Saremo noi a fornirtelo.



    <h1>Quanto impiega l’ordine ad essere spedito?</h1>

    Tutti gli ordini effettuati dal lunedì al venerdì impiegano un tempo di lavorazione dalle 12 alle 24 ore salvo imprevisti es. prodotto danneggiato da sostituire o in arrivo da nostro fornitore.

    Tutti gli ordini effettuati sabato e domenica andranno in preparazione il lunedì successivo, o se effettuati nei giorni festivi, andranno in lavorazione il primo giorno lavorativo successivo.



    <h1>L’ordine risulta ancora “in preparazione”, quando verrà spedito?</h1>

    Quando l’ordine risulta “in preparazione” vuol dire che il pagamento è stato effettuato e che gli addetti stanno preparando il tuo ordine.

    Una volta consegnato al corriere verrà poi cambiato lo stato in “spedito”. Da quel momento entro 12 ore riceverai in automatico una mail dal corriere per tracciare la spedizione.

    Ricordiamo che gli ordini non vengono lavorati e spediti il sabato, la domenica e tutti i giorni di festività nazionale.

    <h1>Posso pagare in contrassegno?</h1>

    Certo. Puoi pagare in tutta tranquillità al nostro corriere. Il contrassegno ha un costo aggiuntivo di 5,00€.



    <h1>Effettuate spedizioni all’estero?</h1>

    Si, noi di FAIMED.it spediamo in tutta Europa.

    Per conoscere i costi di spedizione ti consigliamo di contattare il servizio clienti all’ indirizzo E-mail assistenza@faimed.it indicando i prodotti che si desidera acquistare e l’indirizzo di spedizione. Saremo noi a fornire un rapido preventivo.

</div>
<jsp:include page="default/footer.jsp"/>
</body>

</html>